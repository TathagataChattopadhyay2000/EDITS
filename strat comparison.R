...
# TODO compare speed of the stratifiedmix vs original stratified, and either wrap it or replace it
# if its 1 row then we just use stratified if more then stratifiedmix
getPostQuantilesStratifiedMix <- function(
  ...
  ## shape_1 and shape_2 are now matrices: K mixture components x J cohorts
  shape_1 <- j_data$a_j + matrix(j_data$r,
                                 nrow = nrow(j_data$a_j),
                                 ncol = ncol(j_data$a_j),
                                 byrow = TRUE)

  shape_2 <- j_data$b_j + matrix(j_data$n - j_data$r,
                                 nrow = nrow(j_data$b_j),
                                 ncol = ncol(j_data$b_j),
                                 byrow = TRUE)

  posterior_quantiles <- matrix(NA_real_,
                                nrow = length(quantiles),
                                ncol = ncol(j_data$a_j))

  ## posterior weights per cohort
  w_post <- matrix(NA_real_,
                   nrow = length(j_data$w),
                   ncol = ncol(j_data$a_j))

  for (j in seq_len(ncol(j_data$a_j))) {

    log_fk <- lbeta(shape_1[, j], shape_2[, j]) - lbeta(j_data$a_j[, j], j_data$b_j[, j])
    ...
    k <- sample.int(length(j_data$w),
                       size = n_mcmc_iterations,
                       replace = TRUE,
                       prob = w_post[, j])

    p_draw <- stats::rbeta(n_mcmc_iterations,
                           shape1 = shape_1[k, j],
                           shape2 = shape_2[k, j])
  ...
  if (!is.null(calc_differences)) {
    ...
    diff_quantiles <- apply(calc_differences, 1, function (x) {
      ...
      log_fk_1 <- lbeta(shape_1[, x[1]], shape_2[, x[1]]) - lbeta(j_data$a_j[, x[1]], j_data$b_j[, x[1]])
      log_w_1  <- log(j_data$w) + log_fk_1
      w_post_1 <- exp(log_w_1 - max(log_w_1))
      w_post_1 <- w_post_1 / sum(w_post_1)

      log_fk_2 <- lbeta(shape_1[, x[2]], shape_2[, x[2]]) - lbeta(j_data$a_j[, x[2]], j_data$b_j[, x[2]])
      log_w_2  <- log(j_data$w) + log_fk_2
      w_post_2 <- exp(log_w_2 - max(log_w_2))
      w_post_2 <- w_post_2 / sum(w_post_2)

      comp_1 <- sample.int(length(j_data$w),
                           size = n_mcmc_iterations,
                           replace = TRUE,
                           prob = w_post_1)

      comp_2 <- sample.int(length(j_data$w),
                           size = n_mcmc_iterations,
                           replace = TRUE,
                           prob = w_post_2)

      draw_1 <- stats::rbeta(n_mcmc_iterations,
                             shape1 = shape_1[comp_1, x[1]],
                             shape2 = shape_2[comp_1, x[1]])

      draw_2 <- stats::rbeta(n_mcmc_iterations,
